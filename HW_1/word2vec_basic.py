...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  if data_index + span > N:
        data_index = 0
  batch_one = collections.deque(maxlen=span) #max length
  loop_count = batch_size // num_skips
  repeat = num_skips
  for _ in range(span):
        batch_one.append(data[data_index])
        data_index = (data_index+1) % N # if exceeding the data index start from the beginning
  # print("len of data given " , N)
  for i in range(loop_count):
    target = skip_window
    target_not_add = [target]
    # print("target for num sample ",i, target)
    for num_sample in range(repeat):
          while target in target_not_add:
                target = random.randint(0,span-1)
          target_not_add.append(target) # cannot add current context word again
          # print("num sample target generate for context words ", target)
          # i*num_skips denoting the starting postion of the current data
          # loop_count is the number of times we need to add random context word with the same center to the batch
          labels[i*num_skips + num_sample, 0] = batch_one[skip_window] # these are the context words
          batch[i*num_skips + num_sample] = batch_one[target] # this is the center word
    batch_one.append(data[data_index]) 
    data_index = (data_index+1)%N #increase the data index by one to get the next batch
  data_index = (data_index + N - span) % N
  return batch, labels
